    ...
    try:
        ...
        with indent(4, quote="* "):
            puts(
                colored.blue(
                    "View created : {}".format(
                        ".".join([project, dataset_name, view_name])
                    )
                )
            )
    ...
    for item in view_description["fields"]:
        ...
        view_schema.append(
            bigquery.SchemaField(
                item["field"], item["type"], mode=mode, description=item["description"]
            )
        )
    return view_schema
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="METRIC",
        action="store",
        help="the metric filename to process",
    )
    parser.add_argument("-o", "--output_file", metavar="OUTPUTFILE", action="store")
    parser.add_argument(
        "-v", "--create_view", action="store_true", help="create a view from the metric"
    )
    parser.add_argument(
        "-a", "--all_metrics", action="store_true", help="deploy all views"
    )
    parser.add_argument(
        "-d",
        "--dataset",
        metavar="DATASET",
        action="store",
        help="deploy views to this dataset",
    )
    parser.add_argument(
        "-p",
        "--project",
        metavar="PROJECT",
        action="store",
        help="deploy views to a dataset in this project",
    )
    ...
    if not args.all_metrics:
        with indent(4, quote="* "):
            ...
            puts(
                colored.blue(
                    "Rendering ['{}']".format(args.metric_file.replace(".j2", ""))
                )
            )
